...
// Image-first slides — minimal copy, let the photo speak
const slides = [
  {
...
const wordAnim = {
  ...
  visible: { opacity: 1, y: 0,  skewY: 0,  transition: { duration: 0.52, ease: [0.25, 0.1, 0.25, 1] as const } },
}
...
// ─── Minimal slide content — text sits at the bottom, image fills the frame ───
      ...
      >
        ...
        <motion.div variants={fadeUp} className="mb-4">
          <span className="inline-flex items-center gap-2
                           text-white/55 text-[10px] font-semibold uppercase tracking-[0.28em]">
            <span className="w-4 h-[1px] bg-gold-500 shrink-0" />
            {slide.label}
          </span>
        </motion.div>

        {/* Headline — two short lines */}
        <h2 className="font-playfair font-bold leading-[1.05] mb-6"
        ...
        {/* Single minimal CTA — text link, not a heavy button */}
  ...
  return (
    ...
    >
      ...
      >
        {slides.map((slide, idx) => (
          <SwiperSlide key={idx} className="relative w-full h-full overflow-hidden">
            ...
            {/* Image-first overlays — light top vignette + bottom reading zone only */}
            <div className="absolute inset-0 bg-gradient-to-t
                            from-black/70 via-black/10 to-black/20" />
      ...
      <motion.div
        ...
        className="absolute top-24 right-6 sm:right-10 z-20
      ...
      >
        <button
          ...
          className="w-9 h-9 flex items-center justify-center rounded-full
                     border border-white/25 bg-white/8 hover:bg-gold-500
      ...
      <div className="relative z-10 flex flex-col flex-1 justify-end
                      max-w-7xl mx-auto w-full px-6 sm:px-8 lg:px-10 pb-20">
      ...
      <motion.div
        ...
        className="absolute bottom-8 left-1/2 -translate-x-1/2
                   flex flex-col items-center gap-1.5 text-white/40 z-10"
        aria-hidden="true"
      >
