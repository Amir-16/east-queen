...
/* ─── Companies mega-menu ──────────────────────────────────────────────────── */
function CompaniesMegaMenu({ open }: { open: boolean }) {
  return (
    <AnimatePresence>
      {open && (
        <motion.div
              ...
              <div className="relative overflow-hidden bg-navy-900">
                ...
                <div className="relative h-full flex flex-col justify-between p-6">
                  <div className="w-8 h-8 rounded-full bg-gold-500/20 border border-gold-500/30
                    ...
                    <div className="w-2 h-2 rounded-full bg-gold-400 animate-pulse-dot" />
                  </div>
                  <div>
...
function ProductDropdown({ open, item }: { open: boolean; item: NavItem }) {
  return (
    <AnimatePresence>
      {open && (
        <motion.div
...
export default function Navbar() {
  ...
  const openDrop  = (label: string) => { if (timerRef.current) clearTimeout(timerRef.current); setOpenLabel(label) }
  const closeDrop = () => { timerRef.current = setTimeout(() => setOpenLabel(null), 150) }

  return (
    <>
      {/* Top bar only when not scrolled on home, or always on other pages */}
      <TopBar />
          ...
          <div className="flex items-center justify-between h-[80px]">
            ...
            <nav className="hidden lg:flex items-center gap-0.5">
              {NAV_ITEMS.map((item) => {
                ...
                const baseCls = cn(
                  'relative flex items-center gap-1 px-3 py-2 rounded-lg text-[13px] font-medium',
                ...
                if (isDropdown) {
                  return (
                    ...
                    >
                      ...
                      {item.label === 'Our Companies' && <CompaniesMegaMenu open={isOpen} />}
                      {(item.label === 'Export' || item.label === 'Import') && (
                        <ProductDropdown open={isOpen} item={item} />
                      )}
                    </div>
                  )
                }
                ...
                return (
                  <Link key={item.href ?? item.label} to={item.href!} className={baseCls}>
                    {item.label}
                    {isActive && (
                      <motion.span
                        layoutId="nav-active"
                    ...
                    <motion.div key={item.href ?? item.label} variants={fadeRight}>
                      {isDropdown ? (
                        ...
                        <Link
                          ...
                          onClick={() => setMenuOpen(false)}
                          className={cn(
                            'block px-4 py-3 rounded-xl text-[14px] font-medium transition-colors duration-150',
                            isActive
              ...
              <div className="p-4 bg-slate-50 border-t border-slate-100 space-y-3">
                <a
                  ...
                  className="flex items-center gap-2.5 px-3 py-2.5 bg-white rounded-xl
                             border border-slate-200 text-slate-700 text-sm hover:border-gold-200
                             transition-colors"
                >
